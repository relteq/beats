...
figure('Position', [0, scrsz(2), 1024, 960], 'DefaultSurfaceEdgeColor', 'none');
subplot(2, 3, 1);
pcolor(adens);
...
figure('Position', [0, scrsz(2), 1024, 960]);
subplot(2, 3, 1);
plot(mean(densdiff, 2));
xlim([0, size(densdiff, 1)]);
title('Mean Absolute Density Error, veh/mile/lane');
subplot(2, 3, 2);
plot(mean(denserr, 2));
xlim([0, size(denserr, 1)]);
title('Mean Density Error, %');
subplot(2, 3, 3);
plot(std(denserr, 0, 2));
xlim([0, size(denserr, 1)]);
title('Density Error Spread, %');
subplot(2, 3, 4);
plot(mean(flowdiff, 2));
xlim([0, size(flowdiff, 1)]);
title('Mean Absolute Flow Error, veh/hr/lane');
subplot(2, 3, 5);
plot(mean(flowerr, 2));
